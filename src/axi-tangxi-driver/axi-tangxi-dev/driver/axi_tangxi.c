...
int axitangxi_psddr_plddr_loopback(struct axitangxi_device *dev,
  ...
  printk("tx_data_ps_ptr: 0x%x, rx_data_ps_ptr: 0x%x, tx_data_pl_ptr: 0x%x, "
         "rx_data_pl_ptr: 0x%x, burst_size: %ld\n",
...
void mm2s_ps_data_s2mm_pl(uint32_t tx_data_ps_addr, uint32_t rx_data_pl_addr,
                          uint32_t burst_size) {
  printk("DMA_MM2S_PS_ADDR: 0x%x\n", dma_mm2s_ps_addr);
  // DMA_MM2S_PS_ADDR
  writel(tx_data_ps_addr, dma_mm2s_ps_addr);

  printk("DMA_MM2S_PS_SIZE: 0x%x\n", dma_mm2s_ps_size);
  // DMA_MM2S_PS_SIZE
  writel(burst_size - 1, dma_mm2s_ps_size);

  printk("DMA_S2MM_PL_ADDR: 0x%x\n", dma_s2mm_pl_addr);
  // DMA_S2MM_PL_ADDR
  writel(rx_data_pl_addr, dma_s2mm_pl_addr);

  printk("DMA_S2MM_PL_SIZE: 0x%x\n", dma_s2mm_pl_size);
  // DMA_S2MM_PL_SIZE
